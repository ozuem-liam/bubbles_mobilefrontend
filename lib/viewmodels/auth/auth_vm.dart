...
class UserAuthViewModel extends BaseViewModel {
  ...
  Future login({required String email, required String password}) async {
    setBusy(true);
    final res =
        await ref.read(userAuthServiceProvider).loginCustomer(email, password);
  ...
  Future initiateResetPasswords(
    ...
    setBusy(true);
    final res =
        await ref.read(userAuthServiceProvider).initiateResetPassword(email);
  ...
  Future verifyResetOTP(
    ...
    setBusy(true);
    final res =
        await ref.read(userAuthServiceProvider).verifyResetOTP(token: token);
  ...
  changePassword(
    ...
    setBusy(true);
    final res = await ref.read(userAuthServiceProvider).changePassword(
        password: password,
  ...
  getbubblesDashboardDetails() async {
    final res = await ref.read(userAuthServiceProvider).getCustomerDashboard();
  ...
  getInstallerNotification() async {
    ...
    notifyListeners();
    final res =
        await ref.read(userAuthServiceProvider).getCustomerNotifications();
  ...
  uploadbubblesPicture() async {
    ...
    try {
      ...
      if (pickedImage == null) {
        ...
        final res = await ref.read(userAuthServiceProvider).uploadFiles(
